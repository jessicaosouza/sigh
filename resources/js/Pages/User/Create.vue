<template>
    <Head>
        ...
        <meta type="description" content="Cadastrar Usuário" head-key="description">
    </Head>
    <div class="md:mx-auto xl:mx-auto 2xl:mx-auto 2xl:max-w-7xl xl:max-w-7xl lg:max-w-5xl md:max-w-3xl sm:max-w-xl pt-16 xs:mx-2">
        <div class="flex">
            ...
            <h1 class="text-3xl xl:mx-10 2xl:mx-0 font-bold mb-2 text-gray-800">Cadastro de Usuário</h1>
        </div>

        <div v-if="(form.errors.hasOwnProperty('uploaded_file') && !form.errors.uploaded_file) && (form.errors.hasOwnProperty('created_user') && form.errors.created_user)">
            <div class="w-full bg-yellow-200 rounded-lg">
                <p class="pl-2 py-2 text-yellow-600">O usuário foi criado com sucesso, <Link class="hover:underline hover:text-indigo-400" :href="`/users/${this.$page.props.errors.created_user}/show`">clique aqui para visualizá-lo</Link>. Porém, houve um erro ao realizar upload do arquivo. Por favor, contate a equipe de desenvolvimento.</p>
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            addFile(event){
                this.form.cnh_file = event.target.files[0]
            },
            submit(){
